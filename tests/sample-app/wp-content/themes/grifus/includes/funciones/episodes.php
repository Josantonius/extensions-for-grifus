		...
		if (($_POST["se"] != NULL) && ($_POST["te"] != NULL)) {
			...
			if (($ids != NULL) && ($dtemporada != NULL)) {
				...
				for ($cont = 1; $cont <= $sdasd; $cont++) {
					...
					wp_set_post_terms($post_id, $castor, 'episodegueststar', false);
					wp_set_post_terms($post_id, $director, 'episodedirector', false);
					wp_set_post_terms($post_id, $year, 'episodeyear', false);
					...
					add_post_meta($post_id, "img_episode", ($still_path), true);
