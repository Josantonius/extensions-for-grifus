...
<script>
    ...
    jQuery.each(tmdbdata, function(key, val) {
        ...
        if (key == "air_date") {
            jQuery('#new-tag-episodeyear').val(val.slice(0, 4));
        }
        ...
        if (key == "crew") {
            ...
            jQuery('#new-tag-episodedirector').val(crew_d);
        }
        if (key == "guest_stars") {
            var star_a = "";
            ...
            jQuery('#new-tag-episodegueststar').val(star_a);
        ...
        jQuery.getJSON(url + input + "?" + idioma + apikey, function(tmdbdata) {
            ...
            jQuery.each(tmdbdata, function(key, item) {
                if (key == "name") {
					...
					jQuery('#title').val(item + " " + temp + "x" + epis);
                }
                if (key == "poster_path") {
