    ...
    jQuery.each(tmdbdata, function(key, val) {
        ...
        if (key == "genres") {
            ...
            jQuery.each(tmdbdata.genres, function(i, item) {
                genr += "" + item.name + ", ";
                genr1 = item.name;
                jQuery('input[name=newtvshows_categories]').val(genr1);
                jQuery('#tvshows_categories-add-submit').trigger('click');
                jQuery('#tvshows_categories-add-submit').prop("disabled", false);
                jQuery('input[name=newtvshows_categories]').val("");
            });
        ...
        if (key == "first_air_date") {
            jQuery('#new-tag-tvyear').val(val.slice(0, 4));
        }
        if (key == "created_by") {
            var crea = "";
            ...
            jQuery('#new-tag-tvcreator').val(crea);
        }
        if (key == "production_companies") {
            ...
            jQuery('#new-tag-tvstudio').val(pro);
        }
        if (key == "networks") {
            var net = "";
            ...
            jQuery('#new-tag-tvnetworks').val(net);
        }
        jQuery.getJSON(url + input + "/credits?" + apikey, function(tmdbdata) {
            jQuery.each(tmdbdata, function(key, val) {
                if (key == "cast") {
                    ...
                    jQuery('#new-tag-tvcast').val(valCast);
                }
            });
